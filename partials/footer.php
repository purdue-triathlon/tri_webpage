  <!-- ======= Footer ======= -->

  <footer id="footer">
    <div class="footer-top">
      <div class="container">
        <div class="row">

          <div class="col-lg-3 col-md-6 footer-info">
            <img src="assets/img/logo.png" alt="TheEvenet">
            <p>Purdue triathlon is a recognized Purdue University Club Sport and part of the USA Triathlon Collegiate Club program. Since 2002 Purdue Triathlon has provided students an inclusive environment to meet friends and the opportunity to grow their collegiate multisport interests. Questions? Check out the <a href="#faq">FAQs page</a> or <a href="#subscribe">contact us</a> to learn more.</p>
          </div>

          <div class="col-lg-3 col-md-6 footer-links">
            <h4>Useful Links</h4>
            <ul>
              <li><i class="bi bi-chevron-right"></i> <a href="#hero">Home</a></li>
              <li><i class="bi bi-chevron-right"></i> <a href="#about">About us</a></li>
              <li><i class="bi bi-chevron-right"></i> <a href="#schedule">Schedule</a></li>
              <li><i class="bi bi-chevron-right"></i> <a href="#speakers">Executives</a></li>
              <li><i class="bi bi-chevron-right"></i> <a href="#venue">Boilerman</a></li>
            </ul>
          </div>

          <div class="col-lg-3 col-md-6 footer-links">
            <h4>More Useful Links</h4>
            <ul>
              <li><i class="bi bi-chevron-right"></i> <a href="#hotels">Other races</a></li>
        <!--      <li><i class="bi bi-chevron-right"></i> <a href="#gallery">Gallery</a></li> -->
              <li><i class="bi bi-chevron-right"></i> <a href="https://youtu.be/Pcg9rCp_E8g?si=SnWDgsQq1YqFdxMo">Gallery</a></li>
              <li><i class="bi bi-chevron-right"></i> <a href="#supporters">Sponsors</a></li>
              <li><i class="bi bi-chevron-right"></i> <a href="#faq">FAQs</a></li>
              <li><i class="bi bi-chevron-right"></i> <a href="#subscribe">Email list</a></li>
            </ul>
          </div>

          <div class="col-lg-3 col-md-6 footer-contact">
            <h4>Contact Us</h4>
            <p>
              Arianna Werts, Club President <br>
              <strong>Email:</strong> awerts@purdue.edu<br><br>
              Email a specific executive <a href="#speakers">here</a> <br>
              or, follow us and reach out on social media
            </p>

            <div class="social-links">
              <a href="https://twitter.com/purduetriathlon?lang=en" class="twitter"><i class="bi bi-twitter"></i></a>
              <a href="https://www.facebook.com/purduetriathlon/" class="facebook"><i class="bi bi-facebook"></i></a>
              <a href="https://www.instagram.com/purdue_triathlon/?hl=en" class="instagram"><i class="bi bi-instagram"></i></a>
              <a href="https://www.strava.com/clubs/8497" class="strava"><i class="bi bi-strava"></i></a>
            </div>

          </div>

        </div>
      </div>
    </div>

    <div class="container">
      <div class="copyright">
        &copy; Copyright <strong>Andrew Gray</strong>. 2024. All Rights Reserved
      </div>
      <div class="credits">
        <!--
        All the links in the footer should remain intact.
        You can delete the links only if you purchased the pro version.
        Licensing information: https://bootstrapmade.com/license/
        Purchase the pro version with working PHP/AJAX contact form: https://bootstrapmade.com/buy/?theme=TheEvent
      -->
        Designed by <a href="https://bootstrapmade.com/">BootstrapMade</a>
      </div>
    </div>
  </footer><!-- End  Footer -->

  <a href="#" class="back-to-top d-flex align-items-center justify-content-center"><i class="bi bi-arrow-up-short"></i></a>

  <!-- Vendor JS Files -->
  <script src="assets/vendor/aos/aos.js"></script>
  <script src="assets/vendor/bootstrap/js/bootstrap.bundle.min.js"></script>
  <script src="assets/vendor/glightbox/js/glightbox.min.js"></script>
  <script src="assets/vendor/swiper/swiper-bundle.min.js"></script>
  <script src="assets/vendor/php-email-form/validate.js"></script>

  <!-- Template Main JS File -->
  <script src="assets/js/main.js"></script>